...
1. Calls the `mmap` system call to request a new memory page.
2. The kernel returns a virtual memory address.
3. The program writes to that memory page.
4. The CPU triggers a **page fault**.
5. The Linux kernel allocates a physical page.
6. The kernel updates the **page table mapping**.
...
Understanding how virtual memory, page faults, and demand paging work provides deeper insight into how systems behave under memory pressure or during low-level debugging.
